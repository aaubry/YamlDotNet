		...
		namespace LibYaml {
			...
			yaml_parser_initialize(yaml_parser_t *parser)
			{
				...
				if (!QUEUE_INIT(parser, parser->tokens, INITIAL_QUEUE_SIZE, yaml_token_t))
					goto error;
				if (!STACK_INIT(parser, parser->indents, INITIAL_STACK_SIZE, int))
					goto error;
				if (!STACK_INIT(parser, parser->simple_keys, INITIAL_STACK_SIZE, yaml_simple_key_t))
					goto error;
				if (!STACK_INIT(parser, parser->states, INITIAL_STACK_SIZE, yaml_parser_state_t))
					goto error;
				if (!STACK_INIT(parser, parser->marks, INITIAL_STACK_SIZE, yaml_mark_t))
					goto error;
				if (!STACK_INIT(parser, parser->tag_directives, INITIAL_STACK_SIZE, yaml_tag_directive_t))
					goto error;
			...
			yaml_emitter_initialize(yaml_emitter_t *emitter)
			{
				...
				if (!BUFFER_INIT(emitter, emitter->raw_buffer, OUTPUT_RAW_BUFFER_SIZE))
					goto error;
				if (!STACK_INIT(emitter, emitter->states, INITIAL_STACK_SIZE, yaml_emitter_state_t))
					goto error;
				if (!QUEUE_INIT(emitter, emitter->events, INITIAL_QUEUE_SIZE, yaml_event_t))
					goto error;
				if (!STACK_INIT(emitter, emitter->indents, INITIAL_STACK_SIZE, int))
					goto error;
				if (!STACK_INIT(emitter, emitter->tag_directives, INITIAL_STACK_SIZE, yaml_tag_directive_t))
					goto error;
					...
					int implicit)
			{
				...
				if (tag_directives_start != tag_directives_end) {
					...
					if (!STACK_INIT(&context, tag_directives_copy, INITIAL_STACK_SIZE, yaml_tag_directive_t))
						goto error;
					...
					int start_implicit, int end_implicit)
			{
				...
				if (!STACK_INIT(&context, nodes, INITIAL_STACK_SIZE, yaml_node_t)) goto error;
					...
					yaml_char_t *tag, yaml_sequence_style_t style)
			{
				...
				if (!STACK_INIT(&context, items, INITIAL_STACK_SIZE, yaml_node_item_t)) goto error;
					...
					yaml_char_t *tag, yaml_mapping_style_t style)
			{
				...
				if (!STACK_INIT(&context, pairs, INITIAL_STACK_SIZE, yaml_node_pair_t)) goto error;
