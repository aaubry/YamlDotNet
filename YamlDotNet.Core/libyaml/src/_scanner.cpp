					...
					while (!IS_BLANKZ(parser->buffer))
					{
						...
						else if (!single && CHECK(parser->buffer, '\\'))
						{
							...
							switch (parser->buffer.pointer[1])
							{
								...
								case 'N':   /* NEL (#x85) */
									*(string.pointer++) = (yaml_char_t)'\xC2';
									*(string.pointer++) = (yaml_char_t)'\x85';
									break;

								case '_':   /* #xA0 */
									*(string.pointer++) = (yaml_char_t)'\xC2';
									*(string.pointer++) = (yaml_char_t)'\xA0';
									break;

								case 'L':   /* LS (#x2028) */
									*(string.pointer++) = (yaml_char_t)'\xE2';
									*(string.pointer++) = (yaml_char_t)'\x80';
									*(string.pointer++) = (yaml_char_t)'\xA8';
									break;

								case 'P':   /* PS (#x2029) */
									*(string.pointer++) = (yaml_char_t)'\xE2';
									*(string.pointer++) = (yaml_char_t)'\x80';
									*(string.pointer++) = (yaml_char_t)'\xA9';
									break;
							...
							if (code_length)
							{
								...
								if (value <= 0x7F) {
									*(string.pointer++) = (yaml_char_t)value;
								}
								else if (value <= 0x7FF) {
									*(string.pointer++) = (yaml_char_t)(0xC0 + (value >> 6));
									*(string.pointer++) = (yaml_char_t)(0x80 + (value & 0x3F));
								}
								else if (value <= 0xFFFF) {
									*(string.pointer++) = (yaml_char_t)(0xE0 + (value >> 12));
									*(string.pointer++) = (yaml_char_t)(0x80 + ((value >> 6) & 0x3F));
									*(string.pointer++) = (yaml_char_t)(0x80 + (value & 0x3F));
								}
								else {
									*(string.pointer++) = (yaml_char_t)(0xF0 + (value >> 18));
									*(string.pointer++) = (yaml_char_t)(0x80 + ((value >> 12) & 0x3F));
									*(string.pointer++) = (yaml_char_t)(0x80 + ((value >> 6) & 0x3F));
									*(string.pointer++) = (yaml_char_t)(0x80 + (value & 0x3F));
								}
